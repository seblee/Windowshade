...
void ID_learn(void)
{
    ...
    if (FG_10ms)
    {
        ...
        if (TIMER_err_1s)
            --TIMER_err_1s;
        if (TIMER_Sensor_open_1s)
            --TIMER_Sensor_open_1s;
        if (TIMER_Sensor_close_1s)
            --TIMER_Sensor_close_1s;
        if (TIME_DIP_switch)
            --TIME_DIP_switch;
        if (TIME_OUT_OPEN_CLOSE)
            --TIME_OUT_OPEN_CLOSE; //2015.3.23修改
        if (TIME_Login_EXIT_Button)
            --TIME_Login_EXIT_Button; //2015.3.23修改
        if (TIME_Receiver_LED_OUT)
            --TIME_Receiver_LED_OUT; //2015.3.23修改
        if (time_Login_exit_256)
            --time_Login_exit_256; //2015.3.23修改
        if (Manual_override_TIMER)
            --Manual_override_TIMER; //2015.3.23修改
    ...
    if (FG_10ms)
    {
        ...
        if (TIME_alarm_AUTO)
            --TIME_alarm_AUTO;
        if (TIME_email_Repeat)
            --TIME_email_Repeat;
        if ((TIME_email_send) && (HA_Change_email_time == 0))
            --TIME_email_send;
        if (FLAG_all_Erase_time)
            --FLAG_all_Erase_time;
        if (HA_Change_email_time)
            --HA_Change_email_time;
        if (TIME_Fine_Calibration)
            --TIME_Fine_Calibration;
        if (TIME_send_Faile_notice)
            --TIME_send_Faile_notice; //2015.3.31追加修改 2次发送都失败，SIG绿色LED 1Hz通知
        if (TIME_WIFI_LAN_SELECT)
            --TIME_WIFI_LAN_SELECT;
        if (TIME_one_hour)
            --TIME_one_hour; //2015.1.30追加修改1小时查询一次HA状态
        if (time_APP_Start_up)
            --time_APP_Start_up; //2015.04.27修正
        if (TIME_ID_Login_delay)
            --TIME_ID_Login_delay; //20150430 japan修改1  在切换到登录模式过程中，发现如果有429MHz，有时也会把ID登录进去。
        ...
        if (FLAG_all_Erase == 0) //EEPROM所有数据擦出时，以下CODE不执行
        {
            ...
            if (WIFI_L_Login == 0)
            {
                TIME_Receiver_Login++;
                TIME_Receiver_Login_restrict = 350;
                if (COUNT_Receiver_Login >= 2)
                {
                    FLAG_ID_Login = 1;
                    TIME_Login_EXIT_rest = 6000;
                    FLAG_ID_Login_OK = 0;
                    TIME_ID_Login_delay = 50;
                } //20150430 japan修改1
                if (((FLAG_ID_Erase_Login == 1) && (COUNT_Receiver_Login >= 1)) ||
                    ((FLAG_ID_Login == 1) && (COUNT_Receiver_Login >= 3)))
                    ID_Login_EXIT_Initial();
            ...
            if ((FLAG_ID_Erase_Login == 1) || (FLAG_ID_Login == 1))
            {
                ...
                if ((FLAG_ID_Login_OK == 1) && (FLAG_ID_Login_OK_bank == 0))
                {
                    ...
                    else
                    {
                        ...
                        else if ((FLAG_ID_Login == 1) && (DATA_Packet_ID != 0xFFFFFE))
                            ID_EEPROM_write();
                        else if (FLAG_ID_Erase_Login == 1)
                        {
                            if (FLAG_ID_Erase_Login_PCS == 1)
...
void all_Erase_EEPROM_next(void)
{
    ...
    if (FLAG_all_Erase == 1)
    {
        if ((FLAG_all_Erase_time > 0) && ((WIFI_L_Login != 0) || (WIFI_USBLogin != 0)))
            FLAG_all_Erase = 0;
        else if ((FLAG_all_Erase_time == 0) && (FLAG_all_Erase_loop == 0))
        {
            FLAG_all_Erase_loop = 1;
            TIMER1s = 5000;
            TIMER300ms = 300;
            WIFI_LED_TX = 1;
            WIFI_LED_RX = 0;
        ...
        if (FLAG_all_Erase_loop == 1)
        {
            ...
            if (TIMER1s == 0)
                ; //{FLAG_all_Erase_OK==0;FLAG_all_Erase=0;TIMER300ms=0;TIMER1s=0;SoftReset();}
            else
                ClearWDT(); // Service the WDT
        }
    }
...
void all_Erase_EEPROM(void)
{
    ...
    for (i = 0; i < 35; i++)
    {
        ...
        EMIAL_id_HA[i] = 0;
        EMIAL_id_PCS = 0;
        Email_check_ID[i] = 0x00;
        Emial_check_Control[i] = 0x00;
        HA_Cache_SWITCH_DIP[i] = 0;
        HA_Cache_ha[i] = 0;
        HA_Cache_IDdata[i] = 0;
    }

    if ((WIFI_L_Login == 0) && (WIFI_USBLogin == 0))
    {
        FLAG_all_Erase = 1;
        FLAG_all_Erase_time = 300;
    }
    else
    {
        FLAG_all_Erase = 0;
...
void ID_EEPROM_Initial(void)
{
    ...
    for (i = 0; i < ID_DATA_PCS; i++)
    {
        ...
        Read(&xm[0], 0x3A0 + i, 1);
        if ((xm[0] == 0) || (xm[0] == 0xFF))
            ID_DATA_To[i] = 16; //默认为20秒   20-4
        else
            ID_DATA_To[i] = xm[0];
        Read(&xm[0], 0x3C0 + i, 1);
        if ((xm[0] == 0) || (xm[0] == 0xFF))
            ID_DATA_Tc[i] = 16; //默认为20秒   20-4
        else
            ID_DATA_Tc[i] = xm[0];
#endif
#if defined(__Product_PIC32MX2_Receiver__)
        ClearWDT();
        if ((FLAG_POER_on == 0) && (ID_Receiver_DATA[i] != 0))
        {
            FLAG_POER_on = 1;
            DATA_Packet_ID = ID_Receiver_DATA[i];
        }
#endif
    }
//#if defined(__Product_PIC32MX2_Receiver__)
...
//#endif
    ...
    for (i = 0; i < 13; i++)
    {
        ...
        if (i != 10)
        {
            if (i == 11)
            {
                ...
                xm[0] = 11;
                if (SUN_ON_OFF_seat[0] == 0x00)
                    xm[1] = 0;
                else
                    xm[1] = 1;
                if (SUN_ON_OFF_seat[0] == 0x01)
                    xm[2] = 0x08;
                else if (SUN_ON_OFF_seat[0] == 0x11)
                    xm[2] = 0x80;
                else if (SUN_ON_OFF_seat[0] == 0x10)
                    xm[2] = 0x01;
            }
            else if (i == 12)
            {
                Read(&xm[0], 0x800 + 12 * 128, 103);
                xm[0] = 12;
                if (SUN_ON_OFF_seat[1] == 0x00)
                    xm[1] = 0;
                else
                    xm[1] = 1;
                if (SUN_ON_OFF_seat[1] == 0x01)
                    xm[2] = 0x02;
                else if (SUN_ON_OFF_seat[1] == 0x11)
                    xm[2] = 0xC0;
                else if (SUN_ON_OFF_seat[1] == 0x10)
                    xm[2] = 0x01;
            }
            else
            ...
            if ((xm[0] == 0x00) || (xm[0] > 12) || (xm[1] > 1) || (xm[3] > 0x24) || (xm[4] > 0x60) || (xm[5] > 0x80) || (xm[6] > 0x20))
            {
                ...
                for (j = 1; j < 103; j++)
                    WIFI_alarm_data[m1][j] = 0x00;
            }
            else
            {
                WIFI_alarm_data[m1][0] = m1 + 1;
                for (j = 1; j < 103; j++)
                    WIFI_alarm_data[m1][j] = xm[j];
            }
        }
    }
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        if ((xm[0] == 0x00) || (xm[0] > 10) || (xm[1] > 1) || (xm[2] > 0x24) || (xm[3] > 0x60) || (xm[4] > 0x80))
        {
            ...
            for (j = 1; j < 7; j++)
                Emial_time_data[i][j] = 0x00;
        }
        else
        {
            Emial_time_data[i][0] = i + 1;
            for (j = 1; j < 7; j++)
                Emial_time_data[i][j] = xm[j];
        }
    }
...
void eeprom_IDcheck_0x00(void)
{
    ...
    for (i = 0; i < ID_DATA_PCS; i++)
    {
        if (ID_Receiver_DATA[i] == 0x00)
        {
            INquiry_0x00 = i;
            i = ID_DATA_PCS;
            FLAG_IDCheck_OK_0x00 = 1;
        }
    }
}
...
UINT8 Email_check_TO_APP(void) //2015.4.1修正3 由于APP查询受信器HA状态需要很长的时间，所以追加指令查询缓存在通信机里面的HA状态
{
    ...
    for (i = 0; i < 35; i++)
    {
        if (HA_Cache_IDdata[i] == ID_data_uart_CMD0101_01.IDL)
        {
            Emial_Cache_HA = HA_Cache_ha[i] & 0xFF;
            Emial_Cache_SWITCH = HA_Cache_SWITCH_DIP[i];
            i = 64;
        } //
    }
    if (i == 35)
...
void eeprom_IDcheck_UART(void)
{
    ...
    for (i = 0; i < ID_DATA_PCS; i++)
    {
        if (ID_Receiver_DATA[i] == ID_data.IDL)
        {
            i = ID_DATA_PCS;
            FLAG_IDCheck_OK = 1;
        }
...
void eeprom_IDcheck_CMD0101_01_UART(void)
{
    ...
    for (i = 0; i < ID_DATA_PCS; i++)
    {
        if (ID_Receiver_DATA[i] == ID_data_uart_CMD0101_01.IDL)
        {
            CMD0102_To_or_Tc_place = i;
            i = ID_DATA_PCS;
            FLAG_IDCheck_OK = 1;
        }
        if (FLAG_ID_Erase_Login == 1)
...
void IDcheck_CMD0102_HA_Cache(void) //以下2015.08.21追加
{
    ...
    for (i = 0; i < ID_DATA_PCS; i++)
    {
        if (HA_Cache_IDdata[i] == ID_data.IDL)
        {
            CMD0102_To_or_Tc_HA = i;
            i = ID_DATA_PCS;
            FLAG_IDcheck_CMD0102_HA = 1;
        }
    }
}
...
void eeprom_IDcheck_CMD0111_UART(void)
{
    ...
    for (i = 0; i < ID_DATA_PCS; i++)
    {
        if (ID_Receiver_DATA[i] == ID_data_uart_CMD0111.IDL)
        {
            ID_DATA_To_or_Tc_place = i;
            i = ID_DATA_PCS;
            FLAG_IDCheck_OK = 1;
        }
        if (FLAG_ID_Erase_Login == 1)
...
void eeprom_IDcheck_Multiple(UINT8 value_m)
{
    ...
    for (m_i = 0; m_i < UART1_DATA[value_m]; m_i++)
    {
        ...
        {
            if (ID_Receiver_DATA[m_j] == data_m.IDL)
            {
                FLAG_IDCheck_OK = 1;
                m_j = ID_DATA_PCS;
            }
...
{ //日出日落表格数据DATA设置保存
    ...
    Delay100us(100);         //写周期时间  24LC为5ms,24c或24wc为10ms
    for (i = 11; i < 491; i++)
        Sunrise_sunset_DATA[i - 11] = UART1_DATA[i];
    for (i = 0; i < 15; i++)
    {
...
void SUN_time_get(UINT8 value)
{ //查询某月某地在日出日落表格数据DATA中的OPEN CLOSE时间
    UINT8 xm[10] = {0};
    UINT8 x_h0, x_h1;
...
void alarm_EEPROM_write(void)
{
    ...
    for (i = 0; i < 103; i++)
    {
        Write_Read_compare[i] = 0;
        WIFI_alarm_data_Cache[i] = 0;
    }
    ...
    for (i = 11; i < m2; i++)
        WIFI_alarm_data_Cache[i - 11] = UART1_DATA[i]; //WIFI_alarm_data[m1-1][i-11]=UART1_DATA[i];
    for (j = i; j < 103; j++)
        WIFI_alarm_data_Cache[j] = 0x00; //WIFI_alarm_data[m1-1][j]=0x00;
    ...
    while ((FLAG_Write_Read_compare == 0) && (Write_Read_compare_count < 3))
    {
        ...
        for (i = 0; i < 103; i++)
        {
            if (Write_Read_compare[i] == WIFI_alarm_data_Cache[i])
                FLAG_Write_Read_compare = 1;
            else
            {
                FLAG_Write_Read_compare = 0;
                i = 103;
    ...
    {
        for (i = 0; i < 103; i++)
            WIFI_alarm_data[m1 - 1][i] = WIFI_alarm_data_Cache[i];
        Read_Time(&xm[0]);
        //RTC_Minutes=xm[2]*60+xm[1];
        RTC_Minutes = Hex_Decimal(xm[2], xm[1]); //2014.10.11修改   解决TIMER有时不动作
        NEW_set_alarm_pcf8563(RTC_Minutes);
...
void SUN_EEPROM_write(void)
{
    ...
    for (i = 0; i < 103; i++)
    {
        Write_Read_compare[0][i] = 0;
        Write_Read_compare[1][i] = 0;
        WIFI_alarm_data_Cache[0][i] = 0;
        WIFI_alarm_data_Cache[1][i] = 0;
    }
    for (i = 0; i < 3; i++)
    {
        Write_Read_compare_SUN[i] = 0;
        SUN_ON_OFF_seat_Cache[i] = 0;
    }
    FLAG_Write_Read_compare = 0;
    ...
    if (SUN_ON_OFF_seat_Cache[0] == 0x00)
        WIFI_alarm_data_Cache[0][1] = 0;
    else
        WIFI_alarm_data_Cache[0][1] = 1;
    if (SUN_ON_OFF_seat_Cache[0] == 0x01)
        WIFI_alarm_data_Cache[0][2] = 0x08;
    else if (SUN_ON_OFF_seat_Cache[0] == 0x11)
        WIFI_alarm_data_Cache[0][2] = 0x80;
    else if (SUN_ON_OFF_seat_Cache[0] == 0x10)
        WIFI_alarm_data_Cache[0][2] = 0x01;
    ...
    if (SUN_ON_OFF_seat_Cache[1] == 0x00)
        WIFI_alarm_data_Cache[1][1] = 0;
    else
        WIFI_alarm_data_Cache[1][1] = 1;
    if (SUN_ON_OFF_seat_Cache[1] == 0x01)
        WIFI_alarm_data_Cache[1][2] = 0x02;
    else if (SUN_ON_OFF_seat_Cache[1] == 0x11)
        WIFI_alarm_data_Cache[1][2] = 0xC0;
    else if (SUN_ON_OFF_seat_Cache[1] == 0x10)
        WIFI_alarm_data_Cache[1][2] = 0x01;
    ...
    for (i = 14; i < m2; i++)
    {
        WIFI_alarm_data_Cache[0][i - 9] = UART1_DATA[i];
        WIFI_alarm_data_Cache[1][i - 9] = UART1_DATA[i];
    }
    for (i = m2 - 9; i < 103; i++)
    {
        WIFI_alarm_data_Cache[0][i] = 0x00;
        WIFI_alarm_data_Cache[1][i] = 0x00;
    }
    ...
    while ((FLAG_Write_Read_compare == 0) && (Write_Read_compare_count < 5))
    {
        ...
        for (i = 0; i < 3; i++)
        {
            if (Write_Read_compare_SUN[i] == SUN_ON_OFF_seat_Cache[i])
                FLAG_Write_Read_compare = 1;
            else
            {
                FLAG_Write_Read_compare = 0;
        ...
        if (FLAG_Write_Read_compare == 1)
        {
            ...
            for (i = 0; i < 103; i++)
            {
                if ((Write_Read_compare[0][i] == WIFI_alarm_data_Cache[0][i]) && (Write_Read_compare[1][i] == WIFI_alarm_data_Cache[1][i]))
                    FLAG_Write_Read_compare = 1;
                else
                {
                    FLAG_Write_Read_compare = 0;
    ...
    {
        for (i = 0; i < 3; i++)
            SUN_ON_OFF_seat[i] = SUN_ON_OFF_seat_Cache[i];
        for (i = 0; i < 103; i++)
        {
            WIFI_alarm_data[10][i] = WIFI_alarm_data_Cache[0][i];
            WIFI_alarm_data[11][i] = WIFI_alarm_data_Cache[1][i];
        }
        SUN_time_get(SUN_ON_OFF_seat[2]);
        Read_Time(&xm[0]);
        //RTC_Minutes=xm[2]*60+xm[1];
...
void Emial_time_EEPROM_write(void)
{
    ...
    m2 = 18;
    for (i = 11; i < m2; i++)
        Emial_time_data_Cache[i - 11] = UART1_DATA[i]; //Emial_time_data[m1-1][i-11]=UART1_DATA[i];
    m1 = UART1_DATA[11] - 1;
    m2 = m1 / 4;
    m3 = m1 % 4;
    while ((FLAG_Write_Read_compare == 0) && (Write_Read_compare_count < 3))
    {
        ...
        for (i = 0; i < 7; i++)
        {
            if (Write_Read_compare[i] == Emial_time_data_Cache[i])
                FLAG_Write_Read_compare = 1;
            else
            {
                FLAG_Write_Read_compare = 0;
    ...
    {
        for (i = 0; i < 7; i++)
            Emial_time_data[m1][i] = Emial_time_data_Cache[i];
        Read_Time(&xm[0]);
        //RTC_Minutes=xm[2]*60+xm[1];
        RTC_Minutes = Hex_Decimal(xm[2], xm[1]); //2014.10.11修改   解决TIMER有时不动作
        NEW_set_alarm_pcf8563(RTC_Minutes);
...
void HA_Change_EEPROM_write(void)
{
    ...
    {
        Write_Read_compare[i] = 0;
        HA_Change_send_email_Cache[i] = 0;
    }
    FLAG_Write_Read_compare = 0;
    ...
    while ((FLAG_Write_Read_compare == 0) && (Write_Read_compare_count < 3))
    {
        ...
        for (i = 0; i < 3; i++)
        {
            if (Write_Read_compare[i] == HA_Change_send_email_Cache[i])
                FLAG_Write_Read_compare = 1;
            else
            {
                FLAG_Write_Read_compare = 0;
    ...
    {
        for (i = 0; i < 3; i++)
            HA_Change_send_email[i] = HA_Change_send_email_Cache[i];
    }
}
