...
UINT8 HA_uart[1200] = {
    0xBB, 0x00, 0x20, 0x00, 0x00, 0x00, //头
    0x23, 0x00,                         //长度
    0x00, 0x00,                         //接收邮件地址编号
    0x32, 0x30, 0x00, 0x00,             //年
    0x2E, 0x00, 0x00,                   //月
...
UINT8 Sunrise_sunset_DATA[480] = {
    0x07, 0x00, 0x16, 0x35, 0x06, 0x58, 0x16, 0x46, 0x07, 0x13, 0x17, 0x05, 0x07, 0x31, 0x17, 0x27, 0x07, 0x11, 0x16, 0x21, 0x07, 0x28, 0x17, 0x53, 0x07, 0x09, 0x16, 0x58, 0x07, 0x25, 0x17, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //一月份
...
void RF_test_mode(void)
{
    ...
    while (Receiver_test == 0)
    {
        ...
        if ((Tx_Rx_mode == 2) || (Tx_Rx_mode == 3))
        {
            ...
            if (FG_test_tx_off == 0)
            {
                ...
                dd_set_RX_mode();
                ADF7021_DATA_IO = Input;
            } //dd_set_RX_mode_test()   //2015.3.31修改
              //if(HA_L_signal==0){
            if (Tx_Rx_mode == 2)
                if (TIMER1s == 0)
                {
    ...
    if (HA_ERR_signal == 0)
        HA_Status = 0x83;
    if ((HA_L_signal == 1) && (HA_Sensor_signal == 0))
        HA_Status = 0x81;
    if ((HA_L_signal == 0) || (HA_Sensor_signal == 1))
        HA_Status = 0x82;
...
#if defined(__Product_PIC32MX2_Receiver__)
        for (y = 0; y <= 200; y++)
            ; //特别说明：该行采用XC32的0级优化，即无优化
              //for(y=0;y<=600;y++);   //特别说明：该行采用XC32的1级优化，C编译器优化后延时函数的延时时间被改变了，请注意。
#endif
#if defined(__Product_PIC32MX2_WIFI__)
        for (y = 0; y <= 400; y++)
            ; //特别说明：该行采用XC32的0级优化，即无优化    时间=timer*0.9ms
...
#if defined(__Product_PIC32MX2_WIFI__)
    for (x = 0; x < 4 * timer; x++)
        ; //特别说明：该行采用XC32的0级优化，即无优化    时间=0.7us+timer*1.2us
