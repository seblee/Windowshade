    ...
    for (i = 0; i < 22; i++)
    {
        if (i < 12)
        {
            ...
            else if (WIFI_alarm_data[i][1] == 0x01)
            {
                ...
                if (alarm_Minutes > value0)
                    time_differ = alarm_Minutes - value0;
                //else if(alarm_Minutes<value0)time_differ=alarm_Minutes+1440-value0;
                //else if(alarm_Minutes==value0);//相等时控制输出，后面追加控制输出代码
                else if (alarm_Minutes <= value0)
                    time_differ = alarm_Minutes + 1440 - value0;
                if ((time_differ < Compare_data0) && (time_differ > 0))
                {
                    Compare_data0 = time_differ;
                    xl[1] = WIFI_alarm_data[i][3];
                    xl[0] = WIFI_alarm_data[i][4];
                }
            }
        }
        else
        {
            ...
            else if (Emial_time_data[i1][1] == 0x01)
            {
                ...
                {
                    Compare_data0 = time_differ;
                    xl[1] = Emial_time_data[i1][2];
                    xl[0] = Emial_time_data[i1][3];
                }
            }
        }
    }
...
void Emial_time_OUT(UINT16 value10)
{
    ...
    if (ID_Receiver_DATA[value10] != 0)
    {
        ...
        if (AUTO_SEND_DATA_pcs > 0)
        {
            ...
            {
                if ((AUTO_SEND_DATA[APP_i][0] == xg.IDB[0]) && (AUTO_SEND_DATA[APP_i][1] == xg.IDB[1]) && (AUTO_SEND_DATA[APP_i][2] == xg.IDB[2]))
                    FLAG0 = 1;
            }
            if (FLAG0 == 0)
            {
...
void APP_UART_OUT(UINT16 value10)
{
    ...
    if (UART1_DATA[11] == 0)
    {
        ...
        if (AUTO_SEND_DATA_pcs > 0)
        {
            for (APP_i = 0; APP_i < AUTO_SEND_DATA_pcs; APP_i++)
            {
                if ((AUTO_SEND_DATA[APP_i][0] == UART1_DATA[13 + value10 * 3]) && (AUTO_SEND_DATA[APP_i][1] == UART1_DATA[14 + value10 * 3]) && (AUTO_SEND_DATA[APP_i][2] == UART1_DATA[15 + value10 * 3]))
                    FLAG0 = 1;
            }
            if (FLAG0 == 0)
            {
